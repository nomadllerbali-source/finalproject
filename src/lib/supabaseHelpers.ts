...
export const fromDbSightseeing = (row: SightseeingsRow): Sightseeing => ({
  ...
  vehicleCostsByLocation: row.vehicle_costs_by_location ? (Array.isArray(row.vehicle_costs_by_location) ? row.vehicle_costs_by_location : []) : undefined,
...
export const toDbSightseeing = (s: Sightseeing) => {
  ...
  const dbData = {
    ...
    vehicle_costs_by_location: s.vehicleCostsByLocation || null,
