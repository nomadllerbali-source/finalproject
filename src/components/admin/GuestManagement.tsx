...
import { 
  ...
  UserCheck, UserX, Filter, FileText, Copy, Send
} from 'lucide-react';
...
type GuestTab = 'all' | 'confirmed' | 'followups';
...
const GuestManagement: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        console.error('Error loading guest management data:', error);
      }
    }
  ...
  const updateClientFollowUp = (
    ...
    // Update itinerary costs if client has an itinerary
    const updatedClient = updatedClients.find(c => c.id === clientId);
    if (updatedClient) {
      const { getLatestItinerary, updateItinerary } = useData();
      const latestItinerary = getLatestItinerary(clientId);
      if (latestItinerary) {
  ...
  const getTabCount = (tab: GuestTab) => {
    switch (tab) {
      case 'confirmed':
      ...
      <div className="space-y-6">
          ...
          <div className="px-6 py-4 border-b border-slate-200">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-1 bg-slate-100 rounded-lg p-1">
                ...
                >
                  Confirmed Clients ({getTabCount('confirmed')})
                </button>
                <button
            ...
            {filteredClients.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h4 className="text-slate-900 font-medium">
                  {activeTab === 'confirmed' ? 'No confirmed clients yet' :
                   activeTab === 'followups' ? 'No follow-ups scheduled for today' :
                   'No clients yet'}
                </h4>
                <p className="text-slate-500 mt-1">
                  {activeTab === 'confirmed' ? 'Clients with confirmed bookings will appear here.' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-500 text-sm font-medium">Confirmed Bookings</p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-500 text-sm font-medium">Dead Leads</p>
                <p className="text-2xl font-bold text-slate-900">
                  {clients.filter(c => c.followUpStatus?.status === 'dead').length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              {selectedClient.followUpHistory && selectedClient.followUpHistory.length > 0 && (
                <div>
                  <h4 className="font-semibold text-slate-900 mb-3">Follow-up History</h4>
